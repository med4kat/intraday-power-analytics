...
def main():
    with get_connection() as conn:
        with conn.cursor() as cur:
            ...
            cur.execute("""
                SELECT market, ts_utc, load_mw, source_file
                FROM v_load_latest
                ORDER BY market;
            """)
            rows = cur.fetchall()
            ...
            for market, ts_utc, load_mw, source_file in rows:
                ...
                delta_pct = (delta / prev_load * Decimal("100.0")) if (delta is not None and prev_load) else None
